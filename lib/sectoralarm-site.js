...
module.exports = function(RED) {
    ...
    function connect(node, credentials, config) {
        ...
        if (config.numberOfRetries != undefined && config.numberOfRetries != "" && isNumeric(config.numberOfRetries))
            settings.numberOfRetries = config.numberOfRetries;

        if (config.retryDelayInMs != undefined && config.retryDelayInMs != "" && isNumeric(config.retryDelayInMs))
            settings.retryDelayInMs = config.retryDelayInMs;

        return Promise.resolve()
            .then(() => sectoralarm.connect(credentials.email, credentials.password, config.siteId, settings))
    }
